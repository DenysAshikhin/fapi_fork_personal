        ...
        <ResponsiveContainer width="100%" height="100%"  >
            ...
            >
                ...
                {showFries && (
                    ...
                    <YAxis yAxisId="fries" orientation={showHP ? "right" : "left"}
                        // scale={yScale}
                        scale={'linear'}
                        domain={['auto', 'auto']}
                        tickFormatter={(e, index, payload) => {
                ...
                <Tooltip
                    formatter={(value, name, props) => {
                        return [props.payload.originalProduction ? props.payload.originalProduction.toPrecision(3).toString() : props.payload.fries.toPrecision(3).toString(), name];
                    }}
            ...
            /> */}
                        ...
                        {showFries && (
                            <>
                                {graphObjects.top10Fries.map((val, index) => {
                                    if (index > 0) return;
                                    return (<XAxis key={`xAxis${index}_fries`} dataKey="time" hide={true} xAxisId={"fryXAxis" + index} name="time in seconds" />)
                                })
                                }
                ...
                {showRunning && (
                    <>
                        <XAxis dataKey="time" hide={true} xAxisId={"runningProd"} name="time in seconds" />
                        <Line
                            type="monotone"
                            xAxisId={"runningProd"}
                            // xAxisId={"mainTime"}
                            yAxisId="potatoes"
                            data={runningGraphObjects.runningProd.result.result.dataPointsPotatoes}
                            dataKey="production"
                            // dataKey="value2"
...
export default memo(Graph, function (prev, current) {
    ...
    //or if the y-axis scale is changed
        ...
        if (prev.runningGraphObjects.runningProd.prod.notEquals(current.runningGraphObjects.runningProd.prod)) {
            return false
        }
    }
    return isEqual;
